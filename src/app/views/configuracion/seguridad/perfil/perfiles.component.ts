...
export class PerfilesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
     ...
     ******************************************/
    const that = this;
    this.dataService.contar('Rol').subscribe((data: any) => {
      this.cantidad = data;
    });
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        that.dataService.entidadBasicaPaginada('Rol',
  ...
  editar(id): void {
    this.router.navigateByUrl('dummy', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/seguridad/rol'], { queryParams: { id: id } });
    });
  }
  ...
  eliminar(id) {
    ...
    */
    ...
    if (confirm('Seguro de eliminar el registro?')) {
      this.dataService.eliminarObjeto('Rol', id).subscribe((data: any) => {
