...
export class RecursosComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
     ...
     ******************************************/
    const that = this;
    this.dataService.contar('Recurso').subscribe((data: any) => {
      this.cantidad = data;
    });
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (offset === 0)
          offset = 1
        that.dataService.entidadBasicaPaginada('Recurso',
          [dataTablesParameters.columns[0].data, dataTablesParameters.columns[1].data],
          [this.cod.toUpperCase(), this.des.toUpperCase()],
          offset,
      ...
      columns: [ { data: 'codigo' }, { data: 'descripcion' },{}]
    };
  }
  ...
  editar(id): void {
    this.router.navigateByUrl('dummy', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/seguridad/recurso'], { queryParams: { id: id } })
    })
  }
  ...
  eliminar(id) {
    ...
    */
    ...
    if (confirm('Seguro de eliminar el registro?')) {
      this.dataService.eliminarObjeto('Recurso', id).subscribe((data: any) => {       
  ...
  cancelar(): void {
    this.router.navigateByUrl('dummy', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/seguridad/recurso'])
    })
  }
