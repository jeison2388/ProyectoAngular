...
export class InfraestructurasComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
     ...
     ******************************************/
    const that = this;
    this.dataService.contar('Infraestructura').subscribe((data: any) => {
      this.cantidad = data;
    });
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (offset === 0)
          offset = 1
        that.dataService.entidadBasicaPaginada('Infraestructura',
  ...
  editar(id): void {
    this.router.navigateByUrl('dummy', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/taquillas/infraestructura'], { queryParams: { id: id } })
    })
  }
  ...
  eliminar(id) {
    ...
    */
    ...
    if (confirm('Seguro de eliminar el registro?')) {
      this.dataService.eliminarObjeto('Infraestructura', id).subscribe((data: any) => {       
  ...
  cancelar(): void {
    this.router.navigateByUrl('dummy', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/taquillas/infraestructuras'])
    })
  }
