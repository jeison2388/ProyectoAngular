import { Component, OnInit, AfterViewInit, OnDestroy, ViewChild, SecurityContext, Input, EventEmitter, Output } from '@angular/core';
...
@Component({
  selector: 'app-usuariosasig',
  templateUrl: './usuariosasig.component.html',
})
export class UsuariosasigComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @Output() objetoUsuario: EventEmitter<any> = new EventEmitter<any>();
  ...
  ngOnInit() {
     ...
     ******************************************/
    const that = this;
    this.dataService.contar('VistaAsesores').subscribe((data: any) => {
      this.cantidad = data;
    });
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if (offset === 0)
          offset = 1
        that.dataService.entidadBasicaPaginada('VistaAsesores',
          [ dataTablesParameters.columns[0].data, dataTablesParameters.columns[1].data, dataTablesParameters.columns[2].data, dataTablesParameters.columns[3].data],
          [this.ide.toUpperCase(), this.log.toUpperCase(), this.nom.toUpperCase(), this.ape.toUpperCase()],
          offset,
      ...
      columns: [{ data: 'identificacion' }, { data: 'usuario' }, { data: 'nombres' }, { data: 'apellidos' },{}]
    };
  }
  ...
  editar(id): void {
    this.router.navigateByUrl('dummy', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/seguridad/usuario'], { queryParams: { id: id } })
    })
  }
  ...
  eliminar(id) {
    ...
    */
    ...
    if (confirm('Seguro de eliminar el registro?')) {
      this.dataService.eliminarObjeto('PerfilRecurso', id).subscribe((data: any) => {
  ...
  capturarObjeto(item): void{
    this.objetoUsuario.emit(item);
  }
